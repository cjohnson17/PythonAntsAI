...
class Ants():
    ...
    def setup(self, data):
        ...
        self.unseen = np.ones([self.rows,self.cols], dtype=bool)
                ...
                if len(tokens) >= 3:
                    ...
                    if tokens[0] == 'w':
                        ...
                        if tokens[0] == 'a':
                            ...
                            if owner==MY_ANT:
                                self.my_hills_corners[((row + 1) % self.rows, (col + 1) % self.cols)] = True
                                self.my_hills_corners[((row + 1) % self.rows, (col - 1) % self.cols)] = True
                                self.my_hills_corners[((row - 1) % self.rows, (col + 1) % self.cols)] = True
                                self.my_hills_corners[((row - 1) % self.rows, (col - 1) % self.cols)] = True

                                self.my_hills_cross[((row + 1) % self.rows, col)] = True
                                self.my_hills_cross[((row - 1) % self.rows, col)] = True
                                self.my_hills_cross[(row, (col + 1) % self.cols)] = True
                                self.my_hills_cross[(row, (col - 1) % self.cols)] = True
 		self.update_visible()
		self.unseen[self.vision == True] = False
    ...
    def num_corners_occupied(self,loc):
        num = 0
        corners = []
        corners.append( ( (loc[0] + 1) % self.rows, (loc[1] + 1) % self.cols ) )
        corners.append( ( (loc[0] -1) % self.rows, (loc[1] + 1) % self.cols ) )
        corners.append( ( (loc[0] + 1) % self.rows, (loc[1] -1) % self.cols ) )
        corners.append( ( (loc[0] -1) % self.rows, (loc[1] -1) % self.cols ) )
		
        for i in corners:
            if not self.unoccupied(i):
                num += 1
        return num
    ...
    def update_visible(self):
        ...
        self.vision = np.zeros([self.rows,self.cols],dtype=bool)
